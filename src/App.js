...
const TOURNAMENTS_DEFAULT = [
  { id: "t1", label: "Chili Tournament", dates: "May 29 – 30", dateRange: ["2026-05-29", "2026-05-30"] },
  { id: "t2", label: "Cobras (TSE / Grace and Truth) Tournament", dates: "June 26 – 28", dateRange: ["2026-06-26", "2026-06-28"] },
  { id: "t3", label: "Fairport Tournament", dates: "July 17 – 19", dateRange: ["2026-07-17", "2026-07-19"] },
];
...
export default function App() {
  ...
  const [tournaments, setTournaments] = useState(TOURNAMENTS_DEFAULT);
  const [editingTournaments, setEditingTournaments] = useState(false);
  ...
  useEffect(() => {
    (async () => {
      try {
        const [r, g, ro, to] = await Promise.all([
    ...
    <div style={c.app}>
              ...
              <div style={c.card()}>
                ...
                <div style={{marginTop:"16px"}}>
                  ...
                  <textarea
                    ...
                    placeholder="Anything else the coach should know? Conflicts, other sports, travel plans -- Thank you for your support!"
                    value={comments}
        ...
        <div style={c.wrap}>
          ...
          <div style={{display:"flex",gap:"10px",marginBottom:"20px",flexWrap:"wrap"}}>
            {[
              ...
              {l:"Under 9 Players",v:atRiskCount,col:atRiskCount>0?"#dc2626":"#16a34a"},
            ].map(s=>(
          ...
          {tournaments.map(t=>{
            ...
            return (
              <div key={t.id} style={{...c.card({background:"#fffdf0",border:"1px solid #e8d080"})}}>
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:"8px"}}>
                  <div>
                    <span style={{fontWeight:"700",color:TEXT_DARK,fontSize:"1rem"}}>{t.label}</span>
                    <div style={{fontSize:"0.76rem",color:TEXT_MID,fontFamily:"monospace",marginTop:"3px"}}>📅 {t.dates}</div>
                  </div>
                ...
                <div style={{display:"flex",gap:"5px",flexWrap:"wrap",marginTop:"12px"}}>
                  {roster.map(p=>{
                    const a=responses[p]?.availability?.[t.id];
                    const reason=responses[p]?.reasons?.[t.id];
                ...
                {roster.some(p=>responses[p]?.reasons?.[t.id])&&(
                  <div style={{marginTop:"10px",padding:"9px 12px",background:OFF_WHITE,borderRadius:"7px",fontSize:"0.78rem",fontFamily:"monospace",border:`1px solid #e8d080`}}>
                    {roster.filter(p=>responses[p]?.reasons?.[t.id]).map(p=>(
                      <div key={p} style={{color:TEXT_MID,marginBottom:"2px"}}>
                        <span style={{color:TEXT_DARK,fontWeight:"700"}}>{p}:</span> {responses[p].reasons[t.id]}
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div key={game.id} style={c.card({borderTop:`3px solid ${risk?"#dc2626":BLUE}`, background: risk?"#fff5f5":WHITE})}>
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:"8px"}}>
                  <div>
                    ...
                    <span style={c.riskTag(risk)}>{risk?"⚠️ Under 9 Players":"✅ OK"}</span>
      ...
      {tab==="setup" && (
        <div style={c.wrap}>
          ...
          <div style={c.card({borderTop:`3px solid ${GOLD}`})}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"}}>
              <span style={c.lbl}>Tournaments ({tournaments.length})</span>
              {!editingTournaments
                ?<button style={c.smb} onClick={()=>{setDraftTournaments(tournaments.map(t=>({...t})));setEditingTournaments(true);}}>Edit</button>
                :<div style={{display:"flex",gap:"8px"}}>
                  <button style={c.smb} onClick={async()=>{setTournaments(draftTournaments);await dbSet("mustangs_tournaments",draftTournaments);setEditingTournaments(false);}}>Save</button>
                  <button style={c.db} onClick={()=>setEditingTournaments(false)}>Cancel</button>
                </div>
              }
            </div>
            {!editingTournaments
              ?<div style={{fontFamily:"monospace",fontSize:"0.82rem"}}>
                {tournaments.map(t=>(
                  <div key={t.id} style={{padding:"7px 0",borderBottom:`1px solid #f0e88a`,display:"flex",gap:"12px"}}>
                    <span style={{fontWeight:"700",color:TEXT_DARK,minWidth:"140px"}}>{t.label}</span>
                    <span style={{color:TEXT_MID}}>{t.dates}</span>
                  </div>
                ))}
              </div>
              :<div>
                {draftTournaments.map((t,i)=>(
                  <div key={t.id} style={{display:"flex",gap:"7px",marginBottom:"8px",alignItems:"center"}}>
                    <input style={{...c.inp,flex:2}} placeholder="Tournament name" value={t.label}
                      onChange={e=>setDraftTournaments(arr=>{const n=[...arr];n[i]={...n[i],label:e.target.value};return n;})} />
                    <input style={{...c.inp,flex:2}} placeholder="Dates (e.g. May 29 – 30)" value={t.dates}
                      onChange={e=>setDraftTournaments(arr=>{const n=[...arr];n[i]={...n[i],dates:e.target.value};return n;})} />
                    <button style={c.db} onClick={()=>setDraftTournaments(arr=>arr.filter((_,j)=>j!==i))}>✕</button>
                  </div>
                ))}
                <button style={{...c.smb,marginTop:"8px"}} onClick={()=>setDraftTournaments(arr=>[...arr,{id:`t${Date.now()}`,label:"New Tournament",dates:""}])}>+ Add Tournament</button>
              </div>
            }
          </div>
